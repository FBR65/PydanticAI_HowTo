...
agent = Agent(
    model=model,
    deps_type=str,
    # Statischer System-Prompts
    system_prompt="Nutze den Kundennamen wenn Du antwortest.",
)
...
def add_the_users_name(ctx: RunContext[str]) -> str:
    return f"Der Kundenname lautet {ctx.deps}."


# Dynamischer System-Prompt
@agent.system_prompt
def add_the_date() -> str:
    return f"Das Datum ist {date.today()}."


if __name__ == "__main__":
    result = agent.run_sync("Welches Datum haben wir heute?", deps="Frank")
    print(result.output)
